...
func M1(k *chart.K) (*service.Strategy, bool) {
	...
	for i := 0; i < len(k.Candlesticks)-1; i++ {
		c := k.Candlesticks[i]
		fmt.Printf("closed: %v, ma10: %v, ma50: %v, ma150: %v, date: %v\r\n", c.Close, c.MA[chart.Ma_10], c.MA[chart.Ma_50], c.MA[chart.Ma150], c.Date.Format(time.DateOnly))
	}
