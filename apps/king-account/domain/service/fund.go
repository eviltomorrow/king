...
func FundWithInitAccount(ctx context.Context, aliasName, userId string, openingCash decimal.Decimal) error {
	...
	count, err := persistence.FundWithCountByUserId(ctx, mysql.DB, userId)
	if err != nil {
		return err
	}
	...
	_, err = persistence.FundWithInsertOne(ctx, mysql.DB, fund)
	if err != nil {
		return err
	}
...
func FundWithModifyAliasName(ctx context.Context, aliasName string, fundNo string) error {
	...
	_, err := persistence.FundWithUpdateAliasName(ctx, mysql.DB, aliasName, fundNo)
	return err
}
...
func FundWithListByUserId(ctx context.Context, userId string) ([]*Account, error) {
	...
	funds, err := persistence.FundWithSelectManyByUserId(ctx, mysql.DB, userId)
	if err != nil {
		return nil, err
	}
