...
type StorageClient interface {
	ArchiveMetadata(ctx context.Context, opts ...grpc.CallOption) (grpc.ClientStreamingClient[Metadata, ArchiveResponse], error)
...
func (c *storageClient) ArchiveMetadata(ctx context.Context, opts ...grpc.CallOption) (grpc.ClientStreamingClient[Metadata, ArchiveResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Storage_ServiceDesc.Streams[0], Storage_ArchiveMetadata_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[Metadata, ArchiveResponse]{ClientStream: stream}
	return x, nil
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Storage_ArchiveMetadataClient = grpc.ClientStreamingClient[Metadata, ArchiveResponse]
...
type StorageServer interface {
	ArchiveMetadata(grpc.ClientStreamingServer[Metadata, ArchiveResponse]) error
...
func (UnimplementedStorageServer) ArchiveMetadata(grpc.ClientStreamingServer[Metadata, ArchiveResponse]) error {
...
func _Storage_ArchiveMetadata_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(StorageServer).ArchiveMetadata(&grpc.GenericServerStream[Metadata, ArchiveResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Storage_ArchiveMetadataServer = grpc.ClientStreamingServer[Metadata, ArchiveResponse]
