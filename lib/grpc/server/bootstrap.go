...
func NewGrpcBootstrap(opt ...AppOption) *Bootstrap {
	opts := &defaultAppOptions
	for _, o := range opt {
		o.apply(opts)
	}

	return &Bootstrap{appOptions: opts}
}

func (g *Bootstrap) Init() error {
	var (
	...
	if listenHost == "" || listenHost == "0.0.0.0" {
		localIp, err := netutil.GetInterfaceFirst()
		if err != nil {
			return err
		}
		listenHost = localIp
